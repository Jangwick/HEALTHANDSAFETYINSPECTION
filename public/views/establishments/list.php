            ...
            <main class="flex-1 overflow-y-auto p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    ...
                    <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                        ...
                        <div class="text-2xl font-bold text-slate-900"><?= number_format($stats['compliant']) ?></div>
                    </div>
                    <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                        <div class="text-sm font-medium text-rose-600 mb-1 font-bold italic">Non-Compliant</div>
                        <div class="text-2xl font-bold text-slate-900"><?= number_format($stats['non_compliant']) ?></div>
                    </div>
                    <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                        <div class="md:col-span-2">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Name, owner, or permit..." 
                    ...
                    <?php if (!empty($establishments)): ?>
                            ...
                            <table class="w-full text-left border-collapse">
                                ...
                                <tbody class="divide-y divide-slate-100 italic">
                                    <?php foreach ($establishments as $est): ?>
                                        <tr class="hover:bg-slate-50 transition-colors cursor-pointer" onclick="window.location='/views/establishments/view.php?id=<?= $est['establishment_id'] ?>'">
                                            <td class="px-6 py-4">
                                                <div class="font-bold text-slate-900"><?= htmlspecialchars($est['name']) ?></div>
                                                <div class="text-xs text-slate-400 uppercase tracking-tight"><?= htmlspecialchars($est['business_permit_number'] ?? 'No Permit') ?></div>
                                            </td>
                                            <td class="px-6 py-4">
                                                <span class="text-sm text-slate-600"><?= ucwords($est['type']) ?></span>
                                            </td>
                                            <td class="px-6 py-4">
                                                <div class="text-sm text-slate-900 font-medium"><?= htmlspecialchars($est['owner_name']) ?></div>
                                                <div class="text-xs text-slate-500"><?= htmlspecialchars($est['owner_phone'] ?? '') ?></div>
                                            </td>
                                            <td class="px-6 py-4">
                                                <?php
                                                    $statusClasses = [
                                                ...
                                                <span class="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider <?= $class ?>">
                                                    <?= str_replace('_', ' ', $est['compliance_status']) ?>
                                                </span>
                                            </td>
                                            <td class="px-6 py-4">
                                                <div class="text-sm text-slate-600">
                                                    <?= $est['last_inspection_date'] ? date('M d, Y', strtotime($est['last_inspection_date'])) : 'Never' ?>
                                                </div>
                                                <div class="text-xs text-slate-400"><?= $est['inspection_count'] ?> total</div>
                                            </td>
                                            <td class="px-6 py-4 text-right" onclick="event.stopPropagation()">
                                                <div class="flex justify-end space-x-2">
                                                    <a href="/views/establishments/view.php?id=<?= $est['establishment_id'] ?>" class="p-2 text-slate-400 hover:text-blue-600 transition-colors"><i class="fas fa-eye"></i></a>
                                                    <a href="/views/inspections/create.php?establishment_id=<?= $est['establishment_id'] ?>" class="p-2 text-slate-400 hover:text-emerald-600 transition-colors" title="Schedule Inspection"><i class="fas fa-calendar-plus"></i></a>
                                                </div>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                        ...
                        <?php if ($totalPages > 1): ?>
                            <div class="px-6 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
                                <div class="text-sm text-slate-500 italic">Page <?= $page ?> of <?= $totalPages ?></div>
                                <div class="flex space-x-1">
                                    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                                        <a href="?page=<?= $i ?>&search=<?= urlencode($search) ?>&type=<?= $type ?>&status=<?= $status ?>" 
